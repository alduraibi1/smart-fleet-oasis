-- إصلاح المشكلة في target_roles وإنشاء نظام الإشعارات الذكية المحسن
CREATE TABLE IF NOT EXISTS public.smart_notifications (
  ...
  -- المستخدم المستهدف
  user_id UUID,
  target_roles TEXT[], -- الأدوار المستهدفة (text array)
...
CREATE INDEX IF NOT EXISTS idx_smart_notifications_status ON smart_notifications(status);
...
CREATE INDEX IF NOT EXISTS idx_smart_notifications_reference ON smart_notifications(reference_type, reference_id);
...
-- إدراج بعض البيانات التجريبية للإشعارات
INSERT INTO smart_notifications (title, message, type, category, priority, reference_type, reference_id, reference_data, action_required, delivery_channels)
VALUES 
('تنبيه: عقد قريب الانتهاء', 'العقد CR-2024-001 للعميل أحمد محمد سينتهي خلال 3 أيام', 'urgent', 'contract_expiry', 'urgent', 'contract', gen_random_uuid(), '{"contract_number": "CR-2024-001", "customer_name": "أحمد محمد", "days_remaining": 3}', true, ARRAY['email', 'in_app']),
('دفعة مستحقة', 'يوجد دفعة بقيمة 5,000 ر.س مستحقة للعقد CR-2024-002', 'warning', 'payment_due', 'high', 'contract', gen_random_uuid(), '{"amount": 5000, "contract_number": "CR-2024-002"}', true, ARRAY['email', 'sms', 'in_app']),
('إرجاع مركبة مجدول', 'مركبة كامري 2023 مجدولة للإرجاع غداً', 'info', 'vehicle_return', 'medium', 'vehicle', gen_random_uuid(), '{"vehicle_info": "كامري 2023", "return_date": "غداً"}', false, ARRAY['in_app']),
('صيانة مطلوبة', 'مركبة أكورد 2022 تحتاج صيانة دورية خلال الأسبوع القادم', 'warning', 'maintenance', 'medium', 'vehicle', gen_random_uuid(), '{"vehicle_info": "أكورد 2022", "maintenance_type": "دورية"}', true, ARRAY['email', 'in_app']),
('تم تجديد العقد', 'تم تجديد العقد CR-2024-003 بنجاح لمدة 6 أشهر إضافية', 'success', 'contract_expiry', 'low', 'contract', gen_random_uuid(), '{"contract_number": "CR-2024-003"}', false, ARRAY['in_app']);