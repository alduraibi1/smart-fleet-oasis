...
import { useNationalities, Nationality, NationalityFormData } from '@/hooks/useNationalities';

interface EditNationalityDialogProps {
  open: boolean;
...
export const EditNationalityDialog = ({ open, onOpenChange, nationality, onSuccess }: EditNationalityDialogProps) => {
  const { updateNationality } = useNationalities();
  ...
  useEffect(() => {
    if (nationality) {
      setFormData({
        ...
        id_prefix: nationality.id_prefix || '',
        id_length: nationality.id_length,
        id_validation_regex: nationality.id_validation_regex || '',
        is_active: nationality.is_active,
        priority: nationality.priority
      });
    }
  }, [nationality]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateNationality(nationality.id, formData);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>تعديل الجنسية</DialogTitle>
          <DialogDescription>
            تعديل بيانات الجنسية في النظام
          </DialogDescription>
        </DialogHeader>
