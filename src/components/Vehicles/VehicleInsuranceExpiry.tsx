...
interface VehicleInsuranceExpiryProps {
  expiryDate?: string;
  className?: string;
  showLabel?: boolean;
}

export const VehicleInsuranceExpiry: React.FC<VehicleInsuranceExpiryProps> = ({ 
  expiryDate, 
  className = "",
  showLabel = false
}) => {
    ...
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        <Badge variant={getVariant()} className={`flex items-center gap-1 ${getColor()}`}>
          ...
          {showLabel && <span className="font-medium">التأمين:</span>}
          {getMessage()}
        </Badge>
      </div>
    );
  } catch (error) {
    console.error('Error parsing insurance expiry date:', error);
    return null;
  }
};
